(ns behave.dsl.rewriter
  (:require [behave.dsl.helpers :refer [type-from-value parser-error third]]
...
;; TODO:
;; The rewriter should produce edn expression that is by itself
;; writable and maintainable, the go-style DSL should only be an
;; alternative
...
(defn rewrite-dispatch-fn [expr]
  (case (first expr)
    ...
    (:argument-list :argument-expr :expr1 :expr2 :fn-name :fn-stmt :label-decl
                    ...
                    :var-expr :proc-body-stmt :domain-value :sync-name)
...
(defmethod rewrite :model [[_ & stmts]]
  (let [items (map rewrite stmts)
    ...
    (make-model :constants (get grouped behave.ontology.Constant [])
                ...
                :domains   (get grouped behave.ontology.Domain [])
...
(defmethod rewrite :struct-decl [[_ _ name _ _ fields _]]
  (let [name (rewrite name)
...
(defmethod rewrite :domain-decl [[_ _ name _ type _ values _]]
  (let [name (rewrite name)
        type (rewrite type)
        values (rewrite values)]
    (->Domain name type values)))
...
(defmethod rewrite :sync-decl [[_ _ name _ params _]]
  (->Sync name params))
...
(defmethod rewrite :proc-sync-stmt [[_ _ name _ expr _]]
  (let [name (rewrite name)
        expr (rewrite expr)]
    `(:sync ~name ~expr)))
